...
function getBreakPoints() {
	//empty our distances array before setting new breakpoints
...
function activateMenuOnScroll() {
	...
	//height of menu when fixed to the top:
  ...
  if (scrollDistance < SCROLL_DISTANCES[0].position - navHeight - margin || scrollDistance === 0) {
        //Welcome when scrolled to the top/clicked on Welcome menu item
        LINKS[0].classList.add("active");
        return;
    }  else if (scrollLength >= totalHeight) {
        //Activate Reservations in menu when scrolled to the bottom of page
		//
